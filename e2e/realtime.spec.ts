...
test.describe("realtime tool registry", () => {
  ...
  test("instructs Elva to prefer fast tools and delegate real work to coding_task", async ({
    ...
    expect(session.instructions).toContain(
      "call coding_task so the configured coding agent does the work"
    );
    expect(session.instructions).toContain("Current coding provider: cursor.");
    expect(session.instructions).toContain("coding_task delegates work to Cursor.");
