...
Codex App Server は、ユーザーの Codex 設定に従って **コマンド実行やファイル変更の承認リクエスト** を送ります。リクエストが届くと、右側パネルに内容が表示されます。

1. コマンド、作業ディレクトリ、理由、または diff を確認
2. その場限りで許可するなら **`APPROVE`**
3. 同種の操作をセッション中まとめて許可するなら **`SESSION`**
4. 許可しない場合は **`DECLINE`**

右側の `// PATCH` 表示は、Codex 移行前のローカル `propose_patch` ツール用の承認 UI としても残しています。
...
| Codex App Server のコマンド実行 / ファイル変更承認 | 右側の承認 UI に表示し、`APPROVE` / `SESSION` / `DECLINE` を App Server に返す |
